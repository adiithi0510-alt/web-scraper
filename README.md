...
4. save_to_csv(stories, filename) — Writes all extracted data to a .csv file using Python's built-in csv.DictWriter.
5. load_and_display_csv(filename) — Reads back the saved CSV and prints a 5-row preview to confirm the file was written correctly.
6. main() — Orchestrates all the above steps in sequence, with clear numbered output at each stage.
